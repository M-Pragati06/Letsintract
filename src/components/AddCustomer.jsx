...
const AddCustomer = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validate()) {
      setCustomers([...customers, { name: customerData.name, email: customerData.email, phone: customerData.phone }]);
      setCustomerData({ name: "", email: "", phone: "", password: "", district: "", taluka: "", state: "" });
      setErrors({});
      setReferencedBy("");
      setSearchInput("");
  ...
  const handleGetOtp = () => {
    ...
    alert(`OTP sent to ${customerData.phone}`);
  };
      ...
      <div className="Form max-w-xl mx-auto mt-10 p-6 rounded-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center space-x-2">
            <input type="tel" name="phone" placeholder="Phone Number" value={customerData.phone} onChange={handleChange} className="flex-1/2 rounded p-2 border border-[#640D5F]" />
            ...
            <tbody>
              {customers.map((customer, index) => (
                <tr key={index}>
                  ...
                  <td className="border border-[#aa1ba3] p-2">{customer.phone}</td>
                </tr>
              ))}
            </tbody>
          </table>
